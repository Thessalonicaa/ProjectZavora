    ...
    <div class="w-full max-w-7xl h-[85vh] bg-gradient-to-br from-gray-900/80 via-gray-900/80 to-gray-950/80 backdrop-blur-xl rounded-3xl border border-gray-800/50 shadow-2xl flex overflow-hidden relative z-10 min-h-0">
      ...
      <div class="w-full md:w-96 bg-gray-900/60 border-r border-gray-700/50 flex flex-col">
        ...
        <div class="p-8 border-b border-gray-700/50 bg-gradient-to-r from-red-600/30 via-red-600/10 to-transparent backdrop-blur-sm">
          <h1 class="text-4xl font-extrabold bg-gradient-to-r from-red-500 to-orange-500 bg-clip-text text-transparent mb-2">{{ currentLanguage === 'th' ? 'อินบ็อกซ์' : 'Messages' }}</h1>
          <p class="text-gray-400 text-sm">{{ currentLanguage === 'th' ? 'แชทกับผู้ซื้อและผู้ขาย' : 'Chat with buyers and sellers' }}</p>
        </div>
        ...
        <div class="p-4 border-b border-gray-700/50 bg-gray-900/40">
          <input
            v-model="searchQuery"
            type="text"
            :placeholder="t('ค้นหาการสนทนา') || 'Search conversations...'"
        ...
        <div class="flex-1 overflow-y-auto hide-scrollbar">
          ...
          <div v-if="conversations.length === 0" class="p-8 text-center mt-12">
            ...
            <p class="text-gray-400 font-semibold text-lg">{{ currentLanguage === 'th' ? 'ยังไม่มีการสนทนา' : 'No conversations yet' }}</p>
            <p class="text-gray-500 text-sm mt-2">{{ currentLanguage === 'th' ? 'เริ่มแชทกับผู้ขาย!' : 'Start chatting with sellers!' }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="flex-1 flex flex-col bg-gradient-to-b from-gray-950 to-gray-900 min-h-0">
            ...
            <div class="relative">
              ...
              <div v-if="showDeleteMenu" class="absolute right-0 top-12 bg-gray-800 border border-gray-700 rounded-xl shadow-2xl z-50 min-w-48 overflow-hidden">
                ...
                >
                  ...
                  {{ currentLanguage === 'th' ? 'ลบแชท' : 'Delete Chat' }}
                </button>
              </div>
            </div>
          </div>
          ...
          <div class="p-6 border-t border-gray-700/50 bg-gradient-to-t from-gray-950 to-gray-900/50 backdrop-blur-sm">
            <div class="flex items-center gap-3">
              ...
              >
                ...
                <span class="hidden sm:inline">{{ currentLanguage === 'th' ? 'ส่ง' : 'Send' }}</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div v-else class="flex-1 flex items-center justify-center overflow-y-auto min-h-0">
          <div class="text-center">
            ...
            <p class="text-3xl font-bold text-gray-400 mb-2">{{ currentLanguage === 'th' ? 'เลือกการสนทนา' : 'Select a conversation' }}</p>
            <p class="text-gray-500 text-lg">{{ currentLanguage === 'th' ? 'เลือกจากการสนทนาของคุณเพื่อเริ่มแชท' : 'Choose from your conversations to start chatting' }}</p>
          </div>
        </div>
      </div>
    </div>
...
const formatTime = (timestamp) => {
  ...
  try {
    // สร้าง Date object จาก timestamp
    let date = new Date(timestamp)
    
    // ถ้า timestamp เป็นตัวเลข (milliseconds) ให้ใช้โดยตรง
    if (typeof timestamp === 'number') {
      date = new Date(timestamp)
    } 
    // ถ้า timestamp เป็น string ให้แปลง
    else if (typeof timestamp === 'string') {
      // เพิ่ม Z ถ้าไม่มี timezone info
      const timeStr = timestamp.includes('Z') ? timestamp : timestamp + 'Z'
      date = new Date(timeStr)
    }
    
    // ถ้า timestamp ไม่ถูกต้อง
    if (isNaN(date.getTime())) return 'now'
    
    // สร้าง Date object สำหรับเวลาปัจจุบัน (UTC)
    const now = new Date()
...
const createConversationWithSeller = async (sellerUsername) => {
  ...
  if (currentUsername === sellerUsername) {
    alert('⚠️ You cannot contact yourself')
    return
  }
